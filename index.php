...
//create a pre defined username and password since we do not have databate
$acc_username = "gandamokikay";
$acc_password = "matining25";
$acc_fullname = "Jeleane M. Matining";
...
if (isset($_REQUEST['Login_button']) ===true) {
	//get the username and the password from the form and compare to the predefined username and password
		//kapag mali agad ang username
	if($_REQUEST['form_username'] != $acc_username) {
			header("Location:".$url_add." ?notexist");
		}
		//tama ang username mali ang password
		else if ($_REQUEST ['form_username'] == $acc_username && $_REQUEST['form_password'] != $acc_password) {
			header("Location: ".$url_add."?wrongpass");
			// code...
		}
		//tama ag username at password
		      	...
		      	<h3 class="text-center mb-4">MyTumblr Login</h3>
		      		...
		      		<div class="form-group">
		      			...
		      			if(isset($_REQUEST['notexist'])===true) { 
		      				...
		      				echo "<div class ='alert alert-warning' role ='alert'> Still Login. Please <a href= 'account.php'>click here </a> to proceed.</div>";
